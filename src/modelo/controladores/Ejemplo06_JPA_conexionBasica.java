...
public class Ejemplo06_JPA_conexionBasica {
	...
	private static void obtencionUnaSolaEntidad () {
		...
		Curso c = (Curso) em.find(Curso.class, 1);
		
		System.out.println("Curso localizado -> " + c.getId() + " " + c.getDescripcion());
	 ...
	 * 
	 */
	private static void obtencionPrimero () {
		EntityManagerFactory entityManagerFactory = Persistence.createEntityManagerFactory("JPACentroEducativo");

		EntityManager em = entityManagerFactory.createEntityManager();

		Query q = em.createNativeQuery("SELECT * FROM curso order by id limit 1", Curso.class);
